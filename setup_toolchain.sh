...
do
    bin/patchelf --set-interpreter "$interpreter" --set-rpath '$ORIGIN/../../../../lib' "libexec/gcc/${TRIPLE}/${GCC_VERSION}/$f" &> /dev/null || true
done

bin/patchelf --set-rpath '$ORIGIN/../../../../lib' "libexec/gcc/${TRIPLE}/${GCC_VERSION}/liblto_plugin.so" &> /dev/null || true
