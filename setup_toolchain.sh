...
for f in bin/*
do
    bin/patchelf --set-interpreter "$dir/lib/ld-linux-x86-64.so.2" "$f" &> /dev/null || true
done

for f in cc1 cc1plus collect2 g++-mapper-server lto1 lto-wrapper
do
    bin/patchelf --set-interpreter "$dir/lib/ld-linux-x86-64.so.2" "lib/gcc/x86_64-linux-gnu/11/$f" &> /dev/null || true
done
